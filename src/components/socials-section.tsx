...
const SocialCard = ({ link, index }: { link: (typeof socialLinks)[0], index: number }) => {
  ...
  return (
    <a href={link.href} key={index} target="_blank" rel="noopener noreferrer">
      <Card ref={cardRef} className="group bg-card/50 backdrop-blur-lg hover:border-muted-foreground/50 transition-all duration-300 h-48 flex flex-col items-center justify-center text-center p-6 rounded-2xl shadow-lg hover:shadow-primary/10 relative border border-white/10">
        <div 
